"""
sweep_infer_Re_taylor_green.py
------------------------------
Sweep Re_true × Re_init combinations for the Taylor-Green vortex inference.
Reuses the run drivers and observation loaders from main/infer_Re_taylor_green.py.

Observations come from the analytic Taylor-Green solution (default) or a trained
forward PINN (OBS_SOURCE = "synthetic").

Produces:
  figures/infer_Re_sweep_taylor_green.pdf/png
  results/infer_Re_sweep_taylor_green/sweep_results.csv
"""
...
from src.pinn import TaylorGreenPINN, sample_interior
from main.infer_Re_taylor_green import (
    T_MAX,
    WIDTHS,
    load_analytic_observations,
...
OBS_SOURCE = "analytic"      # "analytic" (any Re) or "synthetic" (needs forward ckpt)
...
OUT_DIR     = "figures"
RESULTS_DIR = "results/infer_Re_sweep_taylor_green"
...
def _load_obs(Re_true, x_obs, y_obs, t_obs):
    if OBS_SOURCE == "synthetic":
        return load_synthetic_observations(Re_true, x_obs, y_obs, t_obs)
    return load_analytic_observations(Re_true, x_obs, y_obs, t_obs)
...
def run_inference(Re_true, Re_init, x_obs, y_obs, t_obs, u_obs, v_obs, seed=42, desc=""):
    ...
    x_col, y_col, t_col = sample_interior(k1, N_COL, t_max=T_MAX)

    # ── Phase 1: warm-up (data only) ──────────────────────────────
    warmup_loss = make_warmup_loss_fn(x_obs, y_obs, t_obs, u_obs, v_obs, w_data=W_DATA)
...
def main():
    ...
    for Re_true in RE_TRUE_VALUES:
        ...
        # Fixed observation locations in [0,2π]² × [0, T_MAX]
        key_obs = jax.random.key(99)
        pts = jax.random.uniform(key_obs, (N_OBS, 3))
        x_obs = pts[:, 0] * 2.0 * jnp.pi
        y_obs = pts[:, 1] * 2.0 * jnp.pi
        t_obs = pts[:, 2] * T_MAX

        try:
            u_obs, v_obs = _load_obs(Re_true, x_obs, y_obs, t_obs)
        ...
        for factor in RE_INIT_FACTORS:
            ...
            Re_final, err_pct = run_inference(
                Re_true, Re_init, x_obs, y_obs, t_obs, u_obs, v_obs, desc=desc
            )
    ...
    _plot(results, "Taylor-Green", "taylor_green")
