...
EPOCHS = 5000
N_OBS = 500
W_DATA = 1000.0  # must match warm-up data weight for consistent Re signal
NET_LR = 1e-3
RE_LR = 1e-2
...
def make_warmup_loss_fn(x_in, y_in, x_wall, y_wall, x_obs, y_obs, u_obs, v_obs):
    ...
    def loss_fn(model):
        u_pred, v_pred, _ = eval_uvp_batch_bs_raw(
            model, x_obs, y_obs, X_MIN, X_MAX, H_CHAN
        )
...
def run_inference(Re_true, Re_init, x_obs, y_obs, u_obs, v_obs, seed=42, desc=""):
    ...
    warmup_loss = make_warmup_loss_fn(
        x_in, y_in, x_wall, y_wall, x_obs, y_obs, u_obs, v_obs
    )
    ...
    loss_fn = make_loss_fn(
        x_col, y_col, x_in, y_in, x_wall, y_wall, x_obs, y_obs, u_obs, v_obs
    )
    ...
    with tqdm(
        total=EPOCHS, desc=desc, unit="ep", dynamic_ncols=True, leave=False
    ) as pbar:
        for epoch in range(EPOCHS):
            ...
            if epoch > 0 and epoch % RESAMPLE_EVERY == 0:
                ...
                x_col, y_col = sample_interior_bs(
                    k_col, N_COL, X_MIN, X_MAX, H_STEP, H_CHAN
                )
                loss_fn = make_loss_fn(
                    x_col, y_col, x_in, y_in, x_wall, y_wall, x_obs, y_obs, u_obs, v_obs
                )
            ...
            if (
                epoch > EPOCHS // 2
                and epoch % 100 == 0
                and len(loss_history) >= 2 * PATIENCE_WINDOW
            ):
...
def main():
    ...
    for Re_true in RE_TRUE_VALUES:
        ...
        # Fixed observation locations for all Re_init values at this Re_true
        key_obs = jax.random.key(99)
        pts = jax.random.uniform(key_obs, (N_OBS * 6, 2))
        x_cand = pts[:, 0] * (X_MAX - X_MIN) + X_MIN
        y_cand = pts[:, 1] * H_CHAN
        valid = ~((x_cand < 0.0) & (y_cand < H_STEP))
        x_obs = x_cand[valid][:N_OBS]
        y_obs = y_cand[valid][:N_OBS]

        u_obs, v_obs = load_of_observations(Re_true, x_obs, y_obs)
        print(
            f"  {N_OBS} obs points: u range [{float(u_obs.min()):.3f}, {float(u_obs.max()):.3f}]"
        )
        ...
        for factor in RE_INIT_FACTORS:
            ...
            print(
                f"  {desc}  →  Re_final={Re_final:7.2f}   err={err_pct:6.2f}%   ({elapsed:.0f}s)"
            )
    ...
    rows = [
        f"{r['Re_true']},{r['Re_init']:.1f},{r['factor']:.4f},"
    ...
    axes[0].set_xlabel(
        r"$\mathrm{Re}_{\mathrm{init}}\,/\,\mathrm{Re}_{\mathrm{true}}$", fontsize=11
    )
